...
export default{components:{AppShell,StatePanel},data(){return{id:null,pet:null,loading:true,submitting:false,form:{contact_name:'',contact_phone:'',living_city:'',living_condition:'',experience:'',reason:''},fields:[{key:'contact_name',label:'联系人'},{key:'contact_phone',label:'联系电话'},{key:'living_city',label:'居住城市'},{key:'living_condition',label:'居住条件'},{key:'experience',label:'养宠经验',full:true},{key:'reason',label:'申请理由',full:true}]}},onLoad(q){this.id=q.id;this.load()},methods:{assetUrl,async load(){this.loading=true;try{this.pet=await adoptionApi.detail(this.id)}catch(e){this.pet=null}finally{this.loading=false}},async apply(){if(['contact_name','contact_phone','living_city','living_condition','reason'].some(k=>!this.form[k]))return uni.showToast({title:'请完整填写申请',icon:'none'});this.submitting=true;try{await adoptionApi.apply(this.id,this.form);uni.showToast({title:'领养申请已提交'});setTimeout(()=>this.my(),400)}catch(e){}finally{this.submitting=false}},my(){uni.navigateTo({url:'/pages/adoption/my'})},back(){uni.navigateBack()}}}
</script>
