...
export default{components:{AppShell,StatePanel},data(){return{posts:[],topics:[],error:'',loading:true}},onLoad(){this.load()},methods:{assetUrl,async load(){this.loading=true;this.error='';const result=await Promise.allSettled([communityApi.list(),communityApi.topics()]);if(result[0].status==='fulfilled')this.posts=result[0].value||[];else this.error=result[0].reason.message;if(result[1].status==='fulfilled')this.topics=result[1].value||[];this.loading=false},publish(){uni.navigateTo({url:uni.getStorageSync('access_token')?'/pages/community/publish':'/pages/auth/login'})},detail(id){uni.navigateTo({url:`/pages/community/detail?id=${id}`})}}}
</script>
