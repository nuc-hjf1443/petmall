	...
	<AppShell active="community"><view class="page-container community-page">
		...
		<view v-else class="post-grid">
			<view v-for="(post,index) in posts" :key="post.id||index" class="post card" @click="detail(post.id)">
				<image v-if="post.media?.[0]?.file_url" class="post-image" :src="assetUrl(post.media[0].file_url)" mode="aspectFill"/><view v-else class="post-image post-placeholder">{{['🐕','🐈','🐾','🌿'][index%4]}}</view>
...
<script>
import AppShell from '../../components/AppShell.vue';import StatePanel from '../../components/StatePanel.vue';import{assetUrl,communityApi}from'../../api'
export default{components:{AppShell,StatePanel},data(){return{posts:[],topics:[],error:'',loading:true}},onLoad(){this.load()},methods:{assetUrl,async load(){this.loading=true;this.error='';const result=await Promise.allSettled([communityApi.list(),communityApi.topics({silentNetworkError:true})]);if(result[0].status==='fulfilled')this.posts=result[0].value||[];else this.error=result[0].reason.message;if(result[1].status==='fulfilled')this.topics=result[1].value||[];this.loading=false},publish(){uni.navigateTo({url:uni.getStorageSync('access_token')?'/pages/community/publish':'/pages/auth/login'})},detail(id){uni.navigateTo({url:`/pages/community/detail?id=${id}`})}}}
</script>
