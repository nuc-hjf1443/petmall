...
export default{components:{AppShell,StatePanel},data(){return{outTradeNo:'',payment:null,loading:true}},onLoad(q){this.outTradeNo=q.out_trade_no;this.load()},methods:{money:formatMoney,async load(){this.loading=true;try{this.payment=await paymentApi.result(this.outTradeNo)}catch(e){this.payment=null}finally{this.loading=false}},async query(){this.loading=true;try{this.payment=await paymentApi.query(this.outTradeNo);uni.showToast({title:'结果已更新'})}catch(e){}finally{this.loading=false}},order(){uni.redirectTo({url:`/pages/order/detail?id=${this.payment.order_id}`})}}}
</script>
